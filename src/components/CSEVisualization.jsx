...
const CSEVisualization = () => {
  ...
  const [currentView, setCurrentView] = useState("rankings"); // Toggle between "rankings" and "results"

  // Set initial match ID
  React.useEffect(() => {
    const teamData = selectedTeam === "Sion 1" ? data.sion1 : data.sion2;
    const teamMatch = teamData.matches.find(
  ...
  const currentMatch = currentData.matches.find(
    (m) => m.matchId === selectedMatchId
  );
  const currentBoards = currentData.boards.filter(
      ...
      <div className="bg-white rounded-lg p-4 mb-8 shadow-md">
        <div className="text-center">
          <h2 className="font-bold text-xl mb-2">
            {currentData.info[0]?.league || "?"} - Ronde{" "}
            {currentData.info[0]?.round || "?"}
          </h2>
        </div>
      </div>
              ...
              <tbody>
                {currentData.rankings.map((rank, index) => (
                  <tr
                    key={index}
                    className={`border-b ${
                      rank.team === selectedTeam ? "bg-red-100" : ""
                    }`}
                  >
            ...
            <div className="p-4">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex justify-center items-center">
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center text-white font-bold bg-gray-600`}>

...
const CSEVisualization = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const enhancedMatches = teamMatches.map(match => ({
          ...match,
  ...
  useEffect(() => {
    const loadBoardData = async () => {
      ...
      try {
        const roundData = await ChessDataManager.loadRoundData(currentRound);
        const matchBoards = roundData.boards.filter(b => b.matchId === selectedMatchId);
  ...
  return (
    <div className="container mx-auto px-4">
      ...
      <div className="bg-white rounded-lg p-4 mb-8 shadow-md">
        <div className="text-center">
          <h2 className="font-bold text-xl mb-2">
            {teamInfo?.league?.name || "?"} - Ronde {currentRound}
          </h2>
        </div>
      </div>
              ...
              <tbody>
                {rankings && rankings.map((rank, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2">{ChessDataManager.getTeamName(rank.teamId)}</td>
      ...
      <div className="bg-white rounded-lg overflow-hidden mb-8 shadow-md">
        <div className="bg-gray-800 text-white px-4 py-3">
          <h2 className="font-bold">Sélection de la ronde</h2>
        </div>
        <div className="p-4">
